...
const generateGalaxy = () => {
  ...
  // Multiply by 3 because there are 3 values (x, y, z)
  const positions = new Float32Array(parameters.count * 3)
  // Multiply by 3 because there are 3 values (r, g, b)
  ...
  for (let i = 0; i < parameters.count; i++) {
    ...
    // Position
    const particleRadius = Math.random() * parameters.radius
    const branchAngle =
    ...
    positions[i3 + 0] =
      Math.cos(branchAngle + spinAngle) * particleRadius + randomX
    positions[i3 + 1] = 0 + randomY
    positions[i3 + 2] =
      Math.sin(branchAngle + spinAngle) * particleRadius + randomZ
